...
for filename in ['styles.css','app.js','experience.css','experience.js','schedule.css','schedule.js','motion.css','motion.js','cosmos.svg','manifest.webmanifest','offline.html','icons/icon-192.png','icons/icon-512.png']:
...
def library_page():
    ...
    def photo_carousel(images, extra_class='', label='Фотографії бібліотеки'):
        ...
        slides = ''.join(
            f'<figure class="library-carousel-slide"{ "" if index == 0 else " hidden" }><button type="button" class="library-carousel-open" data-lightbox-open aria-label="Збільшити фото {index + 1}"><img src="{esc(item["src"])}" alt="{esc(item.get("alt") or "Матеріали бібліотеки")}" loading="lazy"></button>{f"<figcaption>{esc(item['caption'])}</figcaption>" if item.get("caption") else ""}</figure>'
            for index, item in enumerate(images))
        return (f'<div class="library-carousel {extra_class}" data-library-carousel aria-label="{esc(label)}">'
                f'<div class="library-carousel-stage"><button type="button" data-carousel-prev class="library-carousel-arrow" aria-label="Попереднє фото">←</button>{slides}'
                f'<button type="button" data-carousel-next class="library-carousel-arrow" aria-label="Наступне фото">→</button></div>'
                f'<div class="library-carousel-controls"><span data-carousel-count aria-live="polite">1 / {len(images)}</span>'
                f'<button type="button" data-carousel-toggle aria-pressed="true">Пауза</button></div></div>')
    ...
    library_html = page_heading('Головна сторінка бібліотеки','Книги, нові надходження, події та документи бібліотеки — в одному просторі.') + f'''<div class="container library-page">
      ...
      <dialog class="library-lightbox" aria-label="Перегляд фотографій"><button type="button" data-lightbox-close class="library-lightbox-close" aria-label="Закрити">×</button><button type="button" data-lightbox-prev class="library-lightbox-prev" aria-label="Попереднє фото">←</button><div class="library-lightbox-content"><img alt="" data-lightbox-image><div class="library-lightbox-tools"><button type="button" data-zoom-out aria-label="Зменшити">−</button><button type="button" data-zoom-reset aria-label="Скинути масштаб">100%</button><button type="button" data-zoom-in aria-label="Збільшити">+</button></div></div><button type="button" data-lightbox-next class="library-lightbox-next" aria-label="Наступне фото">→</button></dialog>
      <script>(function(){{const root=document.querySelector('.library-page');if(!root)return;const tabs=[...root.querySelectorAll('[data-library-tab]')];const panels=tabs.map(tab=>root.querySelector('#'+tab.dataset.libraryTab));function activate(index,focus){{tabs.forEach((tab,i)=>{{const active=i===index;tab.setAttribute('aria-selected',active);tab.tabIndex=active?0:-1;panels[i].hidden=!active}});if(focus)tabs[index].focus()}}tabs.forEach((tab,index)=>{{tab.addEventListener('click',()=>activate(index,false));tab.addEventListener('keydown',event=>{{let next=index;if(event.key==='ArrowRight')next=(index+1)%tabs.length;else if(event.key==='ArrowLeft')next=(index+tabs.length-1)%tabs.length;else if(event.key==='Home')next=0;else if(event.key==='End')next=tabs.length-1;else return;event.preventDefault();activate(next,true)}})}});const lightbox=root.querySelector('.library-lightbox'),lightboxImage=root.querySelector('[data-lightbox-image]');let activeCarousel=null,zoom=1;function showSlide(carousel,index){{const slides=[...carousel.querySelectorAll('.library-carousel-slide')];if(!slides.length)return;const previous=Number(carousel.dataset.index),next=(index+slides.length)%slides.length;carousel.dataset.index=String(next);slides.forEach(slide=>{{slide.hidden=true;slide.classList.remove('library-slide-from-right','library-slide-from-left')}});const target=slides[next];target.hidden=false;target.classList.add(next>=previous?'library-slide-from-right':'library-slide-from-left');carousel.querySelector('[data-carousel-count]').textContent=(next+1)+' / '+slides.length}}root.querySelectorAll('[data-library-carousel]').forEach(carousel=>{{carousel.dataset.index='0';let timer=carousel.querySelectorAll('.library-carousel-slide').length>1?setInterval(()=>{{if(!document.hidden&&!lightbox.open)showSlide(carousel,Number(carousel.dataset.index)+1)}},5000):null;carousel.querySelector('[data-carousel-prev]').addEventListener('click',()=>showSlide(carousel,Number(carousel.dataset.index)-1));carousel.querySelector('[data-carousel-next]').addEventListener('click',()=>showSlide(carousel,Number(carousel.dataset.index)+1));carousel.querySelector('[data-carousel-toggle]').addEventListener('click',event=>{{const button=event.currentTarget;if(timer){{clearInterval(timer);timer=null;button.textContent='Автоперегляд';button.setAttribute('aria-pressed','false')}}else{{timer=setInterval(()=>{{if(!document.hidden&&!lightbox.open)showSlide(carousel,Number(carousel.dataset.index)+1)}},5000);button.textContent='Пауза';button.setAttribute('aria-pressed','true')}}}});const stage=carousel.querySelector('.library-carousel-stage');let touchStart=null,ignoreNextClick=false;stage.addEventListener('pointerdown',event=>{{touchStart={{x:event.clientX,y:event.clientY}}}});stage.addEventListener('pointerup',event=>{{if(!touchStart)return;const dx=event.clientX-touchStart.x,dy=event.clientY-touchStart.y;touchStart=null;if(Math.abs(dx)>48&&Math.abs(dx)>Math.abs(dy)*1.2){{ignoreNextClick=true;showSlide(carousel,Number(carousel.dataset.index)+(dx<0?1:-1));setTimeout(()=>ignoreNextClick=false,450)}}}});stage.addEventListener('click',event=>{{if(ignoreNextClick||!event.target.closest('[data-lightbox-open]'))return;activeCarousel=carousel;zoom=1;syncZoom();updateLightbox();lightbox.showModal()}})}});function updateLightbox(){{const slides=[...activeCarousel.querySelectorAll('.library-carousel-slide')],slide=slides[Number(activeCarousel.dataset.index)],image=slide.querySelector('img');lightboxImage.src=image.currentSrc||image.src;lightboxImage.alt=image.alt;lightbox.querySelector('[data-lightbox-prev]').disabled=slides.length<2;lightbox.querySelector('[data-lightbox-next]').disabled=slides.length<2}}function syncZoom(){{lightboxImage.style.transform='scale('+zoom+')';lightbox.querySelector('[data-zoom-reset]').textContent=Math.round(zoom*100)+'%'}}function stepLightbox(direction){{showSlide(activeCarousel,Number(activeCarousel.dataset.index)+direction);zoom=1;syncZoom();updateLightbox()}}root.querySelector('[data-lightbox-close]').addEventListener('click',()=>lightbox.close());root.querySelector('[data-lightbox-prev]').addEventListener('click',()=>stepLightbox(-1));root.querySelector('[data-lightbox-next]').addEventListener('click',()=>stepLightbox(1));root.querySelector('[data-zoom-in]').addEventListener('click',()=>{{zoom=Math.min(3,zoom+.25);syncZoom()}});root.querySelector('[data-zoom-out]').addEventListener('click',()=>{{zoom=Math.max(1,zoom-.25);syncZoom()}});root.querySelector('[data-zoom-reset]').addEventListener('click',()=>{{zoom=1;syncZoom()}});lightboxImage.addEventListener('dblclick',()=>{{zoom=zoom>1?1:2;syncZoom()}});lightbox.addEventListener('click',event=>{{if(event.target===lightbox)lightbox.close()}});let dialogTouch=null;lightbox.addEventListener('pointerdown',event=>{{dialogTouch={{x:event.clientX,y:event.clientY}}}});lightbox.addEventListener('pointerup',event=>{{if(!dialogTouch||zoom>1)return;const dx=event.clientX-dialogTouch.x,dy=event.clientY-dialogTouch.y;dialogTouch=null;if(Math.abs(dx)>48&&Math.abs(dx)>Math.abs(dy)*1.2)stepLightbox(dx<0?1:-1)}});document.addEventListener('keydown',event=>{{if(!lightbox.open)return;if(event.key==='ArrowLeft')stepLightbox(-1);if(event.key==='ArrowRight')stepLightbox(1)}})}})();</script>
      </div>'''
