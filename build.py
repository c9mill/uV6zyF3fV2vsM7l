...
def header(active=''):
    ...
    <div class="utility"><div class="container"><span>Поліський національний університет</span><div><a href="/документи/">Публічна інформація</a><a href="/викладачу/">Викладачу</a><span class="language">УКР</span></div></div></div>
    ...
    <nav class="desktop-nav" aria-label="Головна навігація">{items}</nav><div class="header-actions"><a class="icon-button" href="/пошук/" aria-label="Пошук на сайті">{icon('search')}</a><a class="button compact" href="/вступнику/">Вступнику {icon('external')}</a><button class="icon-button menu-toggle" aria-expanded="false" aria-controls="mobile-nav" aria-label="Відкрити меню">{icon('menu')}</button></div></div>
...
def news_card(p):
    img = featured(p)
    visual = f'<img src="{img}" alt="{escape(title(p),quote=True)}" loading="lazy" width="640" height="430">' if img else f'<div class="news-placeholder">{icon("book")}<span>ФКБАД</span></div>'
    return f'<article class="news-card"><a href="{ROUTES[p["id"]]}" class="news-image">{visual}</a><div class="news-meta"><time datetime="{p["date"][:10]}">{date(p)}</time><span>Життя коледжу</span></div><h3><a href="{ROUTES[p["id"]]}">{escape(title(p))}</a></h3><a class="text-link" href="{ROUTES[p["id"]]}">Читати новину {icon("arrow")}</a></article>'
PROGRAMS = [
...
def program_cards():
    return '<div class="program-grid">'+''.join(f'<a class="program-card program-{i}" href="/спеціальності/{slug}/"><div class="program-top"><span class="program-label">{short}</span>{icon("external")}</div><div class="program-art">{icon(["file","book","cap"][i])}</div><span class="eyebrow">Освітньо-професійна програма</span><h3>{name}</h3><p>{desc}</p><span class="program-bottom">Дізнатися про програму {icon("arrow")}</span></a>' for i,(slug,name,desc,short,_) in enumerate(PROGRAMS))+'</div>'
...
def documents():
    ...
    return page_heading('Документи','Правила, положення та навчальні матеріали коледжу.')+f'<div class="container page-content"><form class="filter-bar" id="document-filter" role="search"><div class="search-field">{icon("search")}<label class="sr-only" for="document-query">Назва документа</label><input id="document-query" type="search" placeholder="Знайти документ…"></div><label class="select-field"><span class="sr-only">Розділ документів</span><select id="document-category"><option value="">Усі розділи</option>{options}</select></label></form><p class="results-status" id="document-status" aria-live="polite">Документів: {len(DOCUMENTS)}</p><div class="content-columns"><section><div class="document-catalog">{rows}</div><div class="empty-state" id="document-empty" hidden><h2>Документів не знайдено</h2><p>Спробуй іншу назву або обери всі розділи.</p><button class="button" id="reset-documents" type="button">Скинути фільтри</button></div></section>{sidebar("/документи/")}</div></div>'
