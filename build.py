...
def program_cards():
    return '<div class="program-grid">'+''.join(f'<a class="program-card program-{i}" href="/спеціальності/{slug}/"><div class="program-top"><span class="program-label">{short}</span>{icon("external")}</div><div class="program-art"><span class="program-number" aria-hidden="true">0{i+1}</span>{icon(["file","book","cap"][i])}</div><span class="eyebrow">Освітньо-професійна програма</span><h3>{name}</h3><p>{desc}</p><span class="program-bottom">Дізнатися про програму {icon("arrow")}</span></a>' for i,(slug,name,desc,short,_) in enumerate(PROGRAMS))+'</div>'
