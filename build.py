...
def library_page():
    ...
    def photo_carousel(images, extra_class='', label='Фотографії бібліотеки'):
        ...
        return (f'<div class="library-carousel {extra_class}" data-library-carousel aria-label="{esc(label)}">'
                f'<div class="library-carousel-stage">{slides}</div>'
                f'<div class="library-carousel-controls"><button type="button" data-carousel-prev aria-label="Попереднє фото">←</button>'
                f'<span data-carousel-count aria-live="polite">1 / {len(images)}</span>'
                f'<button type="button" data-carousel-next aria-label="Наступне фото">→</button>'
